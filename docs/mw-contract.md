...
Compilation takes an optional `ServiceID`. When set, the set is narrowed at
compile — after the fetch, above the fetcher seam, so it applies to either
source — to the statements this service can act on: each statement keeps only
its actions scoped to that service (`file` keeps `file:getFile` and `file:*`) or
unscoped (`*`), and statements — then policies — left with no action are
dropped. The file service compiles `file:getFile` but never `state:getJobs`. A
